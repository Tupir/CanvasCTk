...
class Checkbox(CanvasCTkWidget):
    """Canvas port of CustomTkinter's ``CTkCheckBox``."""
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        theme = ctk.ThemeManager.theme["CTkCheckBox"]
        ...
        self._checkmark_color = self._check_color_type(theme["checkmark_color"] if checkmark_color is None else checkmark_color)
        ...
        self._track_theme_defaults(
            ...
            checkmark_color=checkmark_color is None,
    ...
    def _draw(self, *_: Any, no_color_updates: bool = False) -> None:
        requires_recoloring = self._draw_engine.draw_rounded_rect_with_border(
            ...
            self._checkbox_height,
            self._corner_radius,
            self._border_width,
        )
        if self._check_state:
            requires_recoloring = self._draw_engine.draw_checkmark(
                self._checkbox_width,
                self._checkbox_height,
                self._checkbox_height * 0.58,
            ) or requires_recoloring
        else:
    ...
    def _refresh_control_colors(self) -> None:
        ...
        checked_color = self._hover_color if self._hovered and self._state == tk.NORMAL else self._fg_color
        if self._check_state:
            fill = self._apply_appearance_mode(checked_color)
            self._canvas.itemconfig("inner_parts", fill=fill, outline=fill)
            self._canvas.itemconfig("border_parts", fill=fill, outline=fill)
            self._canvas.itemconfig("checkmark", fill=self._apply_appearance_mode(self._checkmark_color))
        else:
            inner = self._hover_color if self._hovered and self._state == tk.NORMAL else background
            self._canvas.itemconfig("inner_parts", fill=self._apply_appearance_mode(inner), outline=self._apply_appearance_mode(inner))
    ...
    def _refresh_text(self) -> None:
        ...
        self._canvas.coords(self._text_id, self._checkbox_width + 6, self._current_height / 2)
        self._canvas.itemconfig(self._text_id, text=self._text, font=self._font, fill=self._apply_appearance_mode(text_color))
    ...
    def configure(self, require_redraw: bool = False, **kwargs: Any) -> None:
        ...
        refresh_text = False
        for name in ("corner_radius", "border_width", "checkbox_width", "checkbox_height"):
            if name in kwargs:
                setattr(self, f"_{name}", int(kwargs.pop(name)))
                require_redraw = True
        for name in ("fg_color", "hover_color", "border_color", "checkmark_color", "text_color", "text_color_disabled"):
            if name in kwargs:
        ...
        if "variable" in kwargs:
            ...
            if self._variable is not None and self._variable != "":
                ...
                if checked != self._check_state:
                    self._check_state = checked
                    require_redraw = True
        for name in ("onvalue", "offvalue", "state", "command"):
            if name in kwargs:
                setattr(self, f"_{name}", kwargs.pop(name))
                if name == "state":
